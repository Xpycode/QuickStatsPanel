...
3. **Themes (named presets)** — **spec drafted: `specs/themes.md`** (research-driven: HIG + Apple/WWDC + competitor teardown). Locked choices: **follow-system light+dark** (no per-app toggle, HIG), **presets + custom knobs**, ship **Default/Mono/Vitals/Neon + Custom**, **unified `usageColor(zones:reversed:)` + hysteresis** (replaces `loadColor` *and* the battery `100-percent` hack). Architecture locked: theme rides the existing `AppSettings.shared` `@Observable` singleton (views read `AppSettings.shared.theme.…` in `body`), **not** `@Environment` (not shared across the 3 NSHostingView roots). Refactor surface ~38 `Theme.*` call sites in 4 views + remove forced `.preferredColorScheme(.dark)` (3 views). 4/6 open Qs resolved (flat fill + manual reduce-transparency fallback; density = font/spacing only, `stripHeight` stays sole height source; Default = today's exact dark tokens verbatim; accent follows system). **Plan: `IMPLEMENTATION_PLAN.md`** — 5 waves / 12 tasks (foundation types → `Theme` struct+`AppSettings` chokepoint → migrate 4 views+samplers → Settings picker+Customize → verify). **✅ Wave 1 done** (commit `3fe3bb4`): `Model/Theme/{ColorCodable,ThemePreset,StatusBand}.swift` — sRGB Color↔Codable bridge, preset/density/custom-payload types, `resolveBand` (±5% hysteresis + `reversed` flag that will retire the battery `100-percent` hack). **✅ Wave 2 done** (commit `caf5694`, the chokepoint): `Theme` static enum → **Sendable token struct** (`colors`/`metrics`/`fonts`/`statusStyle` + `color(for:)`, `tint(forPercent:reversed:previous:)`, `make(_:custom:reduceTransparency:)` defining all 4 presets — **Default = today's dark tokens verbatim** — + appearance-adaptive dynamic-NSColor helper, inert until Wave 3 drops forced dark); a **transitional `@MainActor` facade** (`Theme.Colors.*`/`Fonts.*`/`Metrics.*`/`loadColor`) keeps the 4 views compiling untouched. `AppSettings` now carries persisted `themePreset` + `customTheme` (JSON) and a derived `private(set) var theme` rebuilt in `didSet` **and** on a `NSWorkspace` Reduce-Transparency observer (AC-11). **Zero behavior change** (facade → Default = verbatim). **✅ Wave 3 done** (commit `332bbbe`): all 4 views migrated to `AppSettings.shared.theme.*` (read in `body` → live re-render per `NSHostingView` root), the 3 forced `.preferredColorScheme(.dark)` removed (panel now **follows system**, Default embraces light per user choice), tile takes store-resolved `band`+`tint` with a **width-neutral font-weight severity cue** (calm/busy/hot → regular/semibold/heavy; AC-5), status color resolved once at the `visibleStats` chokepoint via `store.tint(for:percent:reversed:)` + an `@ObservationIgnored [StatKind:Band]` hysteresis map (AC-7), **battery `100-percent` hack gone** (now `reversed:true`; AC-6), and the transitional facade **deleted**. Parallel `developer` agents did the 2 independent view migrations (3.3/3.4); coupled core (3.1/3.2/3.5) done in-orchestrator. Clean signed build; synthetic-summon probe confirms the strip renders (780×36) with **no observation loop** from the in-body cache. **✅ Wave 4 done** (commits `8ad4324` [4.1] + `00d9880` [4.2]): `SettingsView` gains an **"Appearance" `Section`** — a `Picker` bound to `settings.themePreset` (Default/Mono/Vitals/Neon/Custom; live-switches via the `didSet→rebuildTheme` Observation path, AC-1) + a `DisclosureGroup("Customize…")` shown when `.custom`: accent/background `ColorPicker`s, opacity + corner-radius `Slider`s, density `Picker`. All five knobs route through a generic `customBinding<T>(WritableKeyPath<ThemeData,T>)` that reads `customTheme ?? .default` and writes the **whole mutated struct** back (the reassignment is what fires persist + live rebuild, AC-9); `colorBinding` composes on top to bridge `Color`↔`CodableColor` (sRGB). `resetToDefaults()` now also clears theme (`.default` + `customTheme = nil`) — the model-side reset deferred from 2.2. Clean build both commits, app launches with no Settings regression. **✅ Wave 5 mostly done** (commits `7e2ee79` [5.1] + `46b87d6` [5.3]): **5.1** added `NSObservationTrackingEnabled=YES` to the hand-authored `Resources/Info.plist` (web-verified Apple-documented macOS-15 opt-in for AppKit auto-observation; the SwiftUI-`body` path already updates live so it's future-proofing, not a fix) — clean-from-scratch build, **0 Swift warnings**, key confirmed in the built bundle. **5.3** ran the `code-reviewer` agent over the whole Themes diff: **no correctness bugs** (binding write-back, color sRGB round-trip, observation-in-`body`, reset path, slider ranges all sound); applied one consistency nit (`customData` readout accessor); rejected the reviewer's "plist key undocumented" flag as stale (web-verified). **5.2 (on-screen AC verification) is the ONLY remaining Themes work — USER:** AC-2/AC-9 persistence already **data-layer-verified** (wrote a `ThemeData` JSON to defaults, relaunched clean under the stored custom theme, restored prefs); AC-6/AC-7 verified in Wave 3. Still needs eyes: **AC-1** live preset switch, **AC-3** system light/dark, **AC-4** contrast, **AC-5/8** Mono greyscale severity, **AC-10** compact legibility, **AC-11** Reduce Transparency. Watch the **light-mode-upgrade nuance** (existing Light-mode users now get a real light Default panel). To test custom from scratch: `defaults delete com.sim.QuickStatsPanel themePreset customTheme`.
