...
### Wave 3 (parallel — migrate consumers; each an independent file)

- [ ] **3.1**: Migrate strip → `Views/StatsStripView.swift`
...
- [ ] **3.2**: Migrate tile + status tint → `Views/StatTileView.swift`
...
- [ ] **3.3**: Migrate detail card → `Views/StatDetailView.swift`
...
- [ ] **3.4**: Migrate first-run hint → `Views/FirstRunHintView.swift`
  - `Theme.*` → `theme.*`; **remove `.preferredColorScheme(.dark)`**; keycap chip tint from theme.
  - Depends on: 2.2 — Success: hint matches active theme.
  - Backpressure: build clean.

- [ ] **3.5**: Store-resolved tint + drop battery hack → `Model/StatDescriptor.swift` (+ `StatsStore`, `Sampling/BatterySampler.swift`)
